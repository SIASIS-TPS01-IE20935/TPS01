...
export default function verificarFueraAñoEscolar(
  ...
  try {
    ...
    const inicioAñoEscolar = typeof fechaInicioAñoEscolar === 'string' 
      ? new Date(fechaInicioAñoEscolar) 
      : fechaInicioAñoEscolar;
    
    const finAñoEscolar = typeof fechaFinAñoEscolar === 'string' 
    ...
    console.log(`Verificando si fecha ${fechaActual.toISOString()} (${actualAnio}-${actualMes+1}-${actualDia}) está fuera del año escolar`);
    console.log(`Año escolar: ${inicioAnio}-${inicioMes+1}-${inicioDia} hasta ${finAnio}-${finMes+1}-${finDia}`);
    ...
    const estaFuera = actualTimestamp < inicioTimestamp || actualTimestamp > finTimestamp;
    
    // Si está fuera, devolver el rango para indicar el período del año escolar
    if (estaFuera) {
      console.log(`¡Fecha ${actualAnio}-${actualMes+1}-${actualDia} está FUERA del año escolar!`);
      return {
    ...
    console.log(`Fecha ${actualAnio}-${actualMes+1}-${actualDia} está DENTRO del año escolar`);
    return false;
  } catch (error) {
    console.error("Error al verificar fuera del año escolar:", error, {
      fechaActual: fechaActual?.toISOString(),
      fechaInicioAñoEscolar: typeof fechaInicioAñoEscolar === 'string' ? fechaInicioAñoEscolar : fechaInicioAñoEscolar?.toISOString(),
      fechaFinAñoEscolar: typeof fechaFinAñoEscolar === 'string' ? fechaFinAñoEscolar : fechaFinAñoEscolar?.toISOString()
    });
